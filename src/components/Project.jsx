...
const Project = () => {
  ...
  return (
    <section id="projects" className="bg-black min-h-screen w-full flex flex-col items-center gap-6 pt-15 px-4">
      <h1 className="text-yellow-300 text-3xl font-bold text-center">PROJECTS</h1>

      <p className="text-center text-gray-300 text-sm sm:hidden mb-4 px-4">
        Explore our amazing projects! Built with passion and modern technologies.
      </p>
        ...
        <ul className="w-full flex gap-2 sm:gap-4 justify-center flex-wrap">
          {categories.map((category, i) => (
            <li
              ...
              className={`cursor-pointer px-3 sm:px-4 py-1 rounded-md text-sm sm:text-base ${selectedCategory === category
      ...
      <div className="relative w-full max-w-6xl px-2 overflow-hidden pb-8 sm:pb-0">
          ...
          <>
            ...
            >
              {filteredProjects.map((project, index) => (
                ...
                >
                  <div className="border border-purple-800 rounded-lg p-2 sm:p-3 bg-gradient-to-b from-gray-900 via-purple-900 to-gray-900 shadow-md shadow-purple-800/50 hover:scale-105 transition-all duration-300 h-full flex flex-col">
                    <div className="w-full h-[180px] sm:h-32 rounded-md overflow-hidden">
                      <img src={project.img} alt={project.title} className="w-full h-full object-cover" />
                    </div>
                    <h2 className="text-yellow-300 text-base sm:text-sm font-semibold text-center mt-2">{project.title}</h2>
                    <p className="text-white text-sm sm:text-xs mt-1 text-center">{project.desc}</p>
                    <div className="flex flex-wrap gap-1 mt-2 justify-center">
                      {project.tags.map((tag, i) => (
                        <span key={i} className="px-2 py-0.5 text-[10px] rounded-full bg-yellow-500/20 text-yellow-300">
                          {tag}
                        </span>
                      ))}
                    </div>
                    <a
                      ...
                      className="block w-full text-center bg-yellow-400 text-black text-xs font-semibold px-3 py-1.5 rounded-md hover:bg-yellow-300 mt-auto"
                    >
                      Live Demo
                    </a>
                  </div>
